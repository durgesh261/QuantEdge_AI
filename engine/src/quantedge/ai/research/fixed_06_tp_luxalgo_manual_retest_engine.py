"""
QuantEdge AI — Fixed +0.60% TP LuxAlgo Manual Proximal-Edge Retest Research Engine.
===================================================================================
Reproduces manual TradingView / LuxAlgo SMC Order Block trading workflow:
- Proximal Boundary Entry: OB_High for Long, OB_Low for Short.
- Structural Distal Stop Loss: OB_Low for Long, OB_High for Short (SL distance = OB width).
- Fixed +-0.60% Market Price Movement Take Profit from actual entry price.
- Maximum intended account risk at SL = 35% of current capital (applied cap: 100x).
- Continuous compounding from $10.00 with 0.08% roundtrip taker fees.
- Global One-Trade-at-a-Time Lock across BTCUSD, ETHUSD, SOLUSD, XRPUSD.
- Detailed tracking of Retest Sequence (1st, 2nd, 3rd+ retest) and OB Age / Latency.
- Zero lookahead: Entry occurs strictly at t >= break_index + 1.
...
@dataclass
class LuxAlgoManualRetestConfig:
    fixed_tp_market_pct: float = 0.60    # Fixed 0.60% market price movement TP
    max_sl_account_risk_pct: float = 35.0 # Max 35% account loss at SL
    applied_leverage_cap: float = 100.0   # Exchange max leverage cap
    fee_rate: float = 0.0008             # 0.08% roundtrip taker fee
    max_holding_bars: int = 72           # 72 hours max holding horizon
    starting_capital: float = 10.0       # Starting portfolio capital
    data_timeframe: str = "1h"           # Canonical 1H data

@dataclass
class LuxAlgoManualTradeRecord:
    trade_id: int
    asset: str
    direction: str
    ob_formation_time: str               # ISO8601 UTC
    bos_time: str                        # ISO8601 UTC
    retest_time: str                     # ISO8601 UTC
    ...
    gross_pnl_usd: float
    pnl_usd: float                       # Net PnL USD
    ending_capital: float
    ob_age_hours: float                  # Hours from BOS confirmation to entry
    retest_number: int                   # 1 for first retest, 2 for second, etc.
...
def run_luxalgo_manual_retest_backtest(
    data_base_dir: Optional[Path] = None,
    config: Optional[LuxAlgoManualRetestConfig] = None,
    symbols: Optional[List[str]] = None,
    start_date: Optional[datetime] = None,
    end_date: Optional[datetime] = None,
) -> Dict[str, Any]:
    """
    Executes the Manual Proximal-Edge LuxAlgo Retest Backtest with deterministic trade lifecycle.
    """
    cfg = config or LuxAlgoManualRetestConfig()
    ...
    for sym in syms:
        ...
        for s in setups:
            ...
            dir_ = s.direction
            # Manual Proximal Edge Retest Formula:
            # LONG (Bullish OB): Entry is at OB_High, SL is at OB_Low
            # SHORT (Bearish OB): Entry is at OB_Low, SL is at OB_High
            if dir_ == "LONG":
                entry_p = top
                sl_p = bot
            ...
            gross_sl_return_pct = applied_lev * sl_dist_pct
    ...
    # Sort chronologically across all assets
    ...
    executed_trades: List[LuxAlgoManualTradeRecord] = []
    skipped_global_lock = []
    no_fill_setups = []
    ...
    # Track retest counts per OB zone
    ob_retest_counts: Dict[str, int] = {}
    ...
    for s in all_candidate_setups:
        ...
        gross_sl_return_pct = s["gross_sl_return_pct"]
        gross_tp_return_pct = s["gross_tp_return_pct"]
        ...
        for b in range(max_b):
            ...
            # Proximal Edge Touch Entry Check:
            # LONG: price pulls back from above and touches OB_High (c_l <= entry_p)
            # SHORT: price pulls back from below and touches OB_Low (c_h >= entry_p)
            if not filled:
                ...
                if not filled:
                    # Invalidation check: distal boundary breached before proximal fill
                    if dir_ == "LONG" and c_l <= sl_p:
                        outcome = "INVALIDATED_BEFORE_FILL"
                        exit_reason = "DISTAL_SL_BREACHED_BEFORE_FILL"
                        narrative = "Setup cancelled: price breached distal SL before proximal entry was filled."
        ...
        if not filled:
            no_fill_setups.append({
                ...
                "asset": asset,
                "decision_dt": dec_dt,
                "reason": outcome if outcome != "NO_FILL" else "Price never retested proximal edge.",
            })
            continue
            
        # PnL & Compounding
        if outcome == "FILLED_TP":
            ret_pct = gross_tp_return_pct
            realized_r = reward_dist / risk_dist
        elif outcome == "FILLED_SL":
            ret_pct = -1.0 * gross_sl_return_pct
            realized_r = -1.0
        else:
            p_diff = (exit_p - entry_p) if dir_ == "LONG" else (entry_p - exit_p)
            realized_r = p_diff / risk_dist
            ret_pct = realized_r * gross_sl_return_pct
        ...
        ob_age_hours = float(fill_bar + 1) if fill_bar is not None else 1.0
        
        ob_key = f"{asset}_{s['ob_id']}"
        retest_num = ob_retest_counts.get(ob_key, 0) + 1
        ob_retest_counts[ob_key] = retest_num
        
        trade = LuxAlgoManualTradeRecord(
            trade_id=len(executed_trades) + 1,
            asset=asset,
            direction=dir_,
            ob_formation_time=to_utc_str(formation_dt),
            bos_time=to_utc_str(dec_dt),
            retest_time=to_utc_str(fill_dt),
            ...
            retest_time_ist=to_ist_str(fill_dt),
            ...
            entry_to_sl_distance_pct=round(s["sl_distance_pct"], 4),
            theoretical_leverage=round(theoretical_lev, 2),
            leverage=round(applied_lev, 2),
            gross_sl_return_pct=round(gross_sl_return_pct, 2),
            gross_tp_return_pct=round(gross_tp_return_pct, 2),
            fees=fees_usd,
            net_return_pct=round((net_pnl_usd / start_bal) * 100.0, 2) if start_bal > 0 else 0.0,
            starting_capital=start_bal,
            position_notional=notional,
            gross_pnl_usd=gross_pnl_usd,
            pnl_usd=net_pnl_usd,
            ending_capital=capital_net,
            ob_age_hours=ob_age_hours,
            retest_number=retest_num,
            outcome=outcome,
            reason_for_entry=f"Price retested proximal edge ({entry_p:.4f}) of confirmed {dir_} OB.",
    ...
    latency_series = tdf["ob_age_hours"] if len(tdf) > 0 else pd.Series([0.0])
    ...
    # Latency breakdown (Section 16A)
    latency_breakdown = {}
    if len(tdf) > 0:
        tdf_copy = tdf.copy()
        bins_lat = [0, 1, 3, 6, 12, 24, 10000]
        labels_lat = ["1h (Immediate)", "2-3h Retest", "4-6h Retest", "7-12h Retest", "13-24h Retest", ">24h Retest"]
        tdf_copy["latency_tier"] = pd.cut(tdf_copy["ob_age_hours"], bins=bins_lat, labels=labels_lat, right=True)
        for tier_name, grp in tdf_copy.groupby("latency_tier", observed=True):
            ...
            latency_breakdown[str(tier_name)] = {
                "trades": g_n, "wins": g_w, "losses": g_l,
                "win_rate_pct": round(g_wr, 2), "total_realized_r": round(g_r, 2),
                "expectancy_r": round(g_exp, 4), "profit_factor": round(g_pf, 2),
            }
            
    # Retest Number Breakdown (Section 16B)
    retest_breakdown = {}
    if len(tdf) > 0:
        for r_num, grp in tdf.groupby("retest_number"):
            r_label = f"Retest {r_num}" if r_num <= 2 else "Retest 3+"
            g_n = len(grp)
            g_w = len(grp[grp["outcome"] == "FILLED_TP"])
            g_l = len(grp[grp["outcome"] == "FILLED_SL"])
            g_wr = (g_w / g_n * 100.0) if g_n > 0 else 0.0
            ...
            g_loss = abs(float(grp[grp["outcome"] == "FILLED_SL"]["realized_r"].sum())) if g_l > 0 else 1.0
            g_pf = (g_gain / g_loss) if g_loss > 0 else 99.0
            retest_breakdown[r_label] = {
                "trades": g_n, "wins": g_w, "losses": g_l,
                "win_rate_pct": round(g_wr, 2), "total_realized_r": round(g_r, 2),
                "expectancy_r": round(g_exp, 4), "profit_factor": round(g_pf, 2),
            }

    # OB Width Breakdown (Section 16C)
    width_breakdown = {}
    if len(tdf) > 0:
        tdf_copy = tdf.copy()
        bins_w = [0, 0.5, 1.0, 2.0, 10000]
        labels_w = ["<0.5%", "0.5-1%", "1-2%", ">2%"]
        tdf_copy["width_tier"] = pd.cut(tdf_copy["ob_width_pct"], bins=bins_w, labels=labels_w, right=True)
        for w_name, grp in tdf_copy.groupby("width_tier", observed=True):
            g_n = len(grp)
            g_w = len(grp[grp["outcome"] == "FILLED_TP"])
            g_l = len(grp[grp["outcome"] == "FILLED_SL"])
            g_wr = (g_w / g_n * 100.0) if g_n > 0 else 0.0
            ...
            g_loss = abs(float(grp[grp["outcome"] == "FILLED_SL"]["realized_r"].sum())) if g_l > 0 else 1.0
            g_pf = (g_gain / g_loss) if g_loss > 0 else 99.0
            width_breakdown[str(w_name)] = {
                "trades": g_n, "wins": g_w, "losses": g_l,
                "win_rate_pct": round(g_wr, 2), "total_realized_r": round(g_r, 2),
                "expectancy_r": round(g_exp, 4), "profit_factor": round(g_pf, 2),
            }
            
    # Direction Breakdown (Section 16E)
    direction_breakdown = {}
    if len(tdf) > 0:
        for dir_name, grp in tdf.groupby("direction"):
            g_n = len(grp)
            g_w = len(grp[grp["outcome"] == "FILLED_TP"])
            g_l = len(grp[grp["outcome"] == "FILLED_SL"])
            g_wr = (g_w / g_n * 100.0) if g_n > 0 else 0.0
            ...
            g_loss = abs(float(grp[grp["outcome"] == "FILLED_SL"]["realized_r"].sum())) if g_l > 0 else 1.0
            g_pf = (g_gain / g_loss) if g_loss > 0 else 99.0
            direction_breakdown[str(dir_name)] = {
                "trades": g_n, "wins": g_w, "losses": g_l,
                "win_rate_pct": round(g_wr, 2), "total_realized_r": round(g_r, 2),
                "expectancy_r": round(g_exp, 4), "profit_factor": round(g_pf, 2),
            }
