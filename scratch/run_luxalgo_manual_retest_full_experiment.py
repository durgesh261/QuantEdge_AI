"""
Full Experiment Runner: Fixed +0.60% TP LuxAlgo Manual Proximal-Edge Retest Research.
Generates all 5 CSVs/JSON deliverables and prints the full diagnostic report.
"""
...
print("1. Running Full Multi-Year LuxAlgo Manual Proximal Retest Backtest (2024-2026)...")
res = run_luxalgo_manual_retest_backtest(data_base_dir=data_dir, config=cfg)

aug_start = datetime(2026, 8, 1, 0, 0, 0, tzinfo=timezone.utc)
print("2. Running August 1-26, 2026 Slice...")
aug_res = run_luxalgo_manual_retest_backtest(
    data_base_dir=data_dir, config=cfg, start_date=aug_start
)

trades = res["trades"]
trades_df = res["trades_df"]
aug_trades = aug_res["trades"]
aug_trades_df = aug_res["trades_df"]

# -------------------------------------------------------------
# 1. Export Complete Full Trades Ledger CSV
# -------------------------------------------------------------
trades_csv_path = docs_ai_dir / "fixed_06_tp_luxalgo_manual_retest_trades.csv"
fieldnames = [
    "trade_id", "asset", "direction", "ob_formation_time", "bos_time",
    "retest_time", "entry_time", "exit_time", "ob_formation_time_ist",
    "bos_time_ist", "retest_time_ist", "entry_time_ist", "exit_time_ist",
    "ob_high", "ob_low", "ob_width", "ob_width_pct", "entry_price",
    "sl_price", "tp_price", "entry_to_sl_distance_pct", "theoretical_leverage",
    "leverage", "gross_sl_return_pct", "gross_tp_return_pct", "fees",
    "net_return_pct", "starting_capital", "position_notional", "gross_pnl_usd",
    "pnl_usd", "ending_capital", "ob_age_hours", "retest_number", "outcome",
    "reason_for_entry", "reason_for_exit", "is_ambiguous", "holding_bars",
    "holding_time_hours", "realized_r", "cumulative_realized_r",
...
with open(trades_csv_path, "w", newline="", encoding="utf-8") as f:
    ...
    for t in trades:
        ...
        d["gross_pnl_usd"] = f"{d['gross_pnl_usd']:.6f}" if abs(d['gross_pnl_usd']) < 1e6 else f"{d['gross_pnl_usd']:.6e}"
        d["fees"] = f"{d['fees']:.6f}" if d['fees'] < 1e6 else f"{d['fees']:.6e}"
        d["pnl_usd"] = f"{d['pnl_usd']:.6f}" if abs(d['pnl_usd']) < 1e6 else f"{d['pnl_usd']:.6e}"
...
# -------------------------------------------------------------
# 2. Export August 1-26, 2026 Trades CSV
# -------------------------------------------------------------
aug_csv_path = docs_ai_dir / "fixed_06_tp_luxalgo_manual_retest_aug_2026.csv"
with open(aug_csv_path, "w", newline="", encoding="utf-8") as f:
    ...
    for t in aug_trades:
        ...
        d["ending_capital"] = f"{d['ending_capital']:.6f}" if d['ending_capital'] < 1e6 else f"{d['ending_capital']:.6e}"
        writer.writerow(d)
print(f"Exported: {aug_csv_path} ({len(aug_trades)} trades)")

# -------------------------------------------------------------
# 3. Export Monthly Progression CSV
# -------------------------------------------------------------
monthly_path = docs_ai_dir / "fixed_06_tp_luxalgo_manual_retest_monthly.csv"
...
# -------------------------------------------------------------
# 4. Export Asset Breakdown CSV
# -------------------------------------------------------------
asset_path = docs_ai_dir / "fixed_06_tp_luxalgo_manual_retest_asset_breakdown.csv"
...
# -------------------------------------------------------------
json_path = docs_ai_dir / "fixed_06_tp_luxalgo_manual_retest_results.json"
json_data = {
    "experiment": "Fixed +0.60% TP LuxAlgo Manual Proximal-Edge Retest Research",
    ...
    "retest_breakdown": res["retest_breakdown"],
    "width_breakdown": res["width_breakdown"],
    "direction_breakdown": res["direction_breakdown"],
    "august_2026_slice": {
...
print("\n" + "=" * 80)
print("LUXALGO MANUAL PROXIMAL-EDGE RETEST SUMMARY (2024-2026)")
print("=" * 80)
