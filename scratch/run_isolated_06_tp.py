"""
Isolated Pair Backtest: Fixed 0.60% TP, Max 100x Leverage, Max 35% SL, $10.00 Starting Capital per pair (August 2026).
"""
...
for sym in symbols:
    ...
    # Run isolated simulation with $10.00 initial capital and 0.60% TP
    capital_gross = 10.0
    capital_net = 10.0
    peak_gross = 10.0
    peak_net = 10.0
    ...
    fixed_tp_pct = 0.60  # 0.60% fixed price target
    ...
    for s in sym_setups:
        ...
        for b in range(max_b):
            ...
            if filled:
                ...
                if hit_tp and hit_sl:
                    ...
                    exit_dt = c_dt
                    exit_p = tp_p
                    narrative = f"Price reached +0.60% TP target at {tp_p:.2f}."
                    break
        ...
        if filled:
            ...
            all_isolated_trades_06.append(trade_rec)
...
summary_06_df = pd.DataFrame([
    ...
    for r in sorted(pair_results_06.values(), key=lambda x: x["ending_capital_net"], reverse=True)
])

print("\n" + "=" * 110)
print("ISOLATED PAIR PERFORMANCE COMPARISON (FIXED 0.60% TP | $10.00 PER PAIR — AUGUST 1–26, 2026)")
print("=" * 110)
print(summary_06_df.to_string(index=False))

# Write CSVs
csv_summary_path = docs_ai_dir / "august_2026_isolated_06_tp_summary.csv"
summary_06_df.to_csv(csv_summary_path, index=False)
print(f"\nWritten: {csv_summary_path}")

csv_all_trades_path = docs_ai_dir / "august_2026_isolated_06_tp_all_trades.csv"
with open(csv_all_trades_path, "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=list(all_isolated_trades_06[0].keys()))
    writer.writeheader()
    writer.writerows(all_isolated_trades_06)
