"""
Multi-Year Full Dataset Backtest (2024–2026):
Fixed 0.60% TP, Max 100x Leverage, Max 35% SL, Starting Capital $10.00.
Evaluates all 4 isolated pairs and the unified multi-pair portfolio across June 2024 to August 2026.
"""
...
# 1. Run Isolated Pairs for Full Multi-Year (2024–2026)
multiyear_isolated_results = {}
for sym in symbols:
    ...
    multiyear_isolated_results[sym] = res

# 2. Run Combined Multi-Pair Portfolio for Full Multi-Year (2024–2026)
multiyear_combined_res = run_simulation(all_setups_combined, start_cap=10.0, fixed_tp_pct=0.60)

# Summary DataFrame
summary_rows = []
for sym, r in multiyear_isolated_results.items():
    summary_rows.append({
...
summary_rows.append({
    ...
    "Start Capital ($)": multiyear_combined_res["starting_capital"],
    "Gross End ($)": multiyear_combined_res["ending_capital_gross"],
    "Gross Return (%)": f"{multiyear_combined_res['gross_return_pct']:+.2f}%",
    "Net End ($)": multiyear_combined_res["ending_capital_net"],
    "Net Return (%)": f"{multiyear_combined_res['net_return_pct']:+.2f}%",
    "Trades": multiyear_combined_res["executed_trades"],
    "Wins": multiyear_combined_res["wins"],
    "Losses": multiyear_combined_res["losses"],
    "Win Rate (%)": f"{multiyear_combined_res['win_rate_pct']:.2f}%",
    "Expectancy (R)": f"{multiyear_combined_res['expectancy_r']:+.4f}R",
    "Total Realized R": f"{multiyear_combined_res['total_realized_r']:+.2f}R",
    "Profit Factor": multiyear_combined_res["profit_factor"],
    "Max DD (Net %)": f"{multiyear_combined_res['max_drawdown_net_pct']:.2f}%",
})

sum_df = pd.DataFrame(summary_rows)

print("\n" + "=" * 110)
print("MULTI-YEAR (2024–2026) FULL DATASET BACKTEST SUMMARY (FIXED 0.60% TP | MAX 100x LEVERAGE | $10.00 BASE)")
print("=" * 110)
print(sum_df.to_string(index=False))

# Year-by-Year Breakdown per Asset
annual_rows = []
for sym, r in multiyear_isolated_results.items():
    tdf = pd.DataFrame(r["trades"])
    tdf["year"] = tdf["entry_time_ist"].str.slice(0, 4)
    for yr, ydf in tdf.groupby("year"):
        ...
        annual_rows.append({
            ...
            "Wins": len(yw),
            "Losses": len(yl),
            "Win Rate (%)": f"{len(yw)/len(ydf)*100:.2f}%",
            "Realized R": f"{ydf['realized_r'].sum():+.2f}R",
            "Expectancy (R)": f"{ydf['realized_r'].mean():+.4f}R",
        })

print("\n" + "=" * 80)
print("YEAR-BY-YEAR BREAKDOWN (2024, 2025, 2026)")
print("=" * 80)
print(pd.DataFrame(annual_rows).to_string(index=False))

# Save CSVs
csv_summary_path = docs_ai_dir / "multiyear_2024_2026_fixed_06_tp_summary.csv"
sum_df.to_csv(csv_summary_path, index=False)
print(f"\nWritten: {csv_summary_path}")

csv_all_trades_path = docs_ai_dir / "multiyear_2024_2026_fixed_06_tp_all_trades.csv"
with open(csv_all_trades_path, "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=list(multiyear_combined_res["trades"][0].keys()))
    writer.writeheader()
    writer.writerows(multiyear_combined_res["trades"])
