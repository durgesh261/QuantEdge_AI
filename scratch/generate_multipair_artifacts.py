"""
Full August 2026 Multi-Pair Concurrent Execution Runner and Artifact Generator.
"""
...
# Run for full August 2026 (Aug 1 - Aug 26)
res_full_aug = run_multipair_sim(all_aug_setups, start_cap=10.0)
full_trades = res_full_aug["executed_trades"]

# Write to CSV
out_csv = docs_ai_dir / "august_2026_multipair_concurrent_trades.csv"
with open(out_csv, "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=list(full_trades[0].keys()))
    writer.writeheader()
    writer.writerows(full_trades)

print(f"Full August 2026 Multi-Pair Concurrent Trades written to {out_csv} ({len(full_trades)} trades)")
